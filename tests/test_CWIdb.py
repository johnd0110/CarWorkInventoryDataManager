...
class CWIDatabaseTests(baseTestSuite):
    ...
    def testValidSelectStatementsSucceeds(self):
        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM CARS", returnColumnNames=False)[0]), 0)

        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM Items", returnColumnNames=False)[0]), 0)

        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM Employees", returnColumnNames=False)[0]), 0)

        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM WorkEfforts", returnColumnNames=False)[0]), 0)

        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM PurchasesHistory", returnColumnNames=False)[0]), 0)

        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM ItemGroupTransactions", returnColumnNames=False)[0]), 0)

        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM ValueEstimates", returnColumnNames=False)[0]), 0)

        self.assertGreater(len(self.db.CWI_executeSQLStatement("SELECT * FROM Purchases", returnColumnNames=False)[0]), 0)

    def testPurchaseHistoryHasStrictlyIncreasingVersions(self):
        purchaseHistoryRes, _ = self.db.CWI_executeSQLStatement('SELECT * FROM PurchasesHistory WHERE purchaseKey = 1 ORDER BY version', returnColumnNames=False)
    ...
    def testPurchaseHistoryHasMonotonicallyIncreasingEffectiveDates(self):
        ...
        sleep(1)
        self.db.CWI_executeSQLStatement('UPDATE Purchases SET cost = 123, taxesPaid = 123, shippingCost = 123, refundAmount = 123 WHERE purchaseKey IN (1)', returnColumnNames=False)

        sleep(1)
        self.db.CWI_executeSQLStatement('UPDATE Purchases SET cost = 23, taxesPaid = 34, shippingCost = 45, refundAmount = 56 WHERE purchaseKey IN (1)', returnColumnNames=False)
    ...
    def testValidUpdateStatementSucceeds(self):
        ...
        self.assertEqual(len(self.db.CWI_executeSQLStatement("UPDATE WorkEfforts SET workEffortDate='2022-11-24', laborHours=13, estimatedPay=140, workType='hub cap replacement' WHERE workEffortKey=1", returnColumnNames=False)[0]), 0)
